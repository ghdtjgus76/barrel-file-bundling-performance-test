 ...
 */
export interface TextField${num}Props {
  label: string;
  placeholder?: string;
  defaultValue?: string;
  value?: string;
...
const TextField${num} = forwardRef<HTMLTextAreaElement, TextField${num}Props>(
  (
    {
      helperText,
      label,
...
TextField${num}.displayName = "TextField${num}";
export default TextField${num};
...
for (let i = 1; i <= 50; i++) {
  const componentDir = path.join(dir, `TextField${i}`);
